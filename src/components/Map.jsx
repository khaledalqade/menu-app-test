...
const MapAdderess = ({ onClose }) => {
  ...
  useEffect(() => {
    // يتم إزالة الكود المتعلق بالعثور على أقرب فرع
  }, [markerPosition]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        // الكود الذي يستخدم localStorage
      
      const token = localStorage.getItem("token");

      const response = await axios.post(
        "https://do-env.xyz/api/v1/address/store",
        dataToSend,
        {
          headers: {
        ...
        <div className="absolute top-[6px] right-[6px] bg-white p-[10px] rounded-[20px] z-30 w-[300px] h-[585px]">
          ...
          <form className="w-full h-[458px] overflow-y-auto overflow-x-hidden">
            ...
            <StyledDropdown
              ...
              defaultOptionText="Address Type" // تغيير النص الافتراضي هنا
            />
            <AutoResizeTextarea
              id="Address_name"
              placeholder=" "
